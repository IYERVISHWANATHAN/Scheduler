...
export function EnhancedCalendarView({
  ...
  // Convert pixel position to time using exact formula from yesterday
  ...
  // Calculate positioned meetings with FINAL ALIGNMENT from yesterday
  const positionedMeetings = useMemo((): PositionedMeeting[] => {
    ...
    dayMeetings.forEach(meeting => {
      ...
      // FINAL ALIGNMENT: 48px header + (timeSlotIndex * 16px) for pixel-perfect positioning
  ...
  const deleteMeetingMutation = useMutation({
    mutationFn: async (meetingId: number) => {
      ...
      return response.text(); // DELETE may return empty response
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/meetings'] });
  ...
  // Mouse event handlers for drag and drop with proper dependencies
  ...
  const handleTimeSlotClickInternal = useCallback((time: string) => {
    const dateString = format(selectedDate, 'yyyy-MM-dd');
      ...
      >
          ...
          return (
            <div key={time}>
              ...
              <div
                ...
                onClick={() => handleTimeSlotClickInternal(time)}
        ...
        {positionedMeetings.map((meeting) => (
          <div
            ...
            onMouseDown={(e) => canViewDetails && handleMouseDown(e, meeting)}
